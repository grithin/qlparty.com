...
class InputFilter{
	...
	static function toDate(&$value,$inOutTz=null){
		...
		return $value = i()->Time($value,$inOutTz)->setZone(Config::$x['timezone'])->date();
	}
	static function toDatetime(&$value,$inOutTz=null){
		$inOutTz = $inOutTz ? $inOutTz : Config::$x['inOutTimezone'];
		return $value = i()->Time($value,$inOutTz)->setZone(Config::$x['timezone'])->datetime();
	}
