...
class Db{
	...
	public $result;
	/// The name given to the db connection on instantiation
	public $name;
	/// Name of the primary database connection
	static $primaryName = 0;
	/// object representing the primary database (usually an instance of this class)
	static $primary;
...
array(
	...
	*/
	static function initialize($connectionInfo,$name=0,$override=false){
		if(!isset(self::$instances[$name]) || $override){
			//add connection
			$class = __class__;
			self::$instances[$name] = new $class();
			self::$instances[$name]->name = $name;
			self::$instances[$name]->connectionInfo = $connectionInfo;
	...
	/// used to translate calls to non existent methods to _method
	...
	*/
	...
	private function quoteIdentity($identity){
		$identity = '"'.$identity.'"';
