...
class Page{
	...
	static function error($message,$name=null,$options=null){
		self::message($message,$name,'error',$options);
	}
	static function success($message=null,$name=null,$context=null,$options=null){
		if(!$message){
			$message = View::pageTitle().' successful';
		}
		self::message($message,$name,'success',$context,$options);
	}
	static function notice($message,$name=null,$context=null,$options=null){
		self::message($message,$name,'notice',$context,$options);
	}
	static function warning($message,$name=null,$context=null,$options=null){
		self::message($message,$name,'warning',$context,$options);
	}
	static function message($message,$name,$type,$context=null,$options=null){
		$context = $context ? $context : self::defaultContext;
	...
	static function fieldError($field,$context=self::defaultContext){
		return self::getMessages('error',$context,$field);
	}
	///checks if there was an error.  Defaults to checking all contexts
	static function errors($context=null){
		return self::getMessages('error',$context);
	}
	///get messages based on context and name
	static function getMessages($type=null,$context=null,$name=null){
		$messages = array();
	...
	@return	null, so use Page::errors() to see if any errors were generated
	*/
...
if($_COOKIE['_PageMessages'] && !Page::$in['_ajax']){
	do{
...
global $page;
if(!$page){//Page class is not loaded with preloader, so controllers may have already set up a $page variable
	$page = new PageObject;
}
Page::$data =& $page;