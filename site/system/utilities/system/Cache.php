...
Underlying cache functions
	set(key,value,+expirySeconds), expirySeconds= 0, lasts till memcached stops
	get(key)
...
class Cache{
	...
	public $cacher;	
	/// Name of the primary cacher object
	static $primaryName = 0;
	/// object representing the primary cacher
	static $primary;
	...
	*/
	...
	//sets primary to some named db object.  Can bypass and directly set $primary if object is not named
	static function setPrimary($name){
		self::$primaryName = $name;
		self::$primary = self::$connections[$name];
	}
	/// used to translate static calls to the primary database instance
	static function __callStatic($name,$arguments){
		if(!self::$primary->cacher){
			self::$primary->connect();
		}
		if(method_exists(self::$primary,$name)){
			return call_user_func_array(array(self::$primary,$name),$arguments);
		}
		//if it doesn't exist on primary connection object, perhaps it exists on underlying cacher object
		return call_user_func_array(array(self::$primary->cacher,$name),$arguments);
	}
