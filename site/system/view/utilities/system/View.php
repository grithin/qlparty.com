<?php
///For handling output and templates, generally assuming http
class View{
	///include alias file
	static $aliases;
	static function initialize(){
		...
		//ensure cookie is set prior to outputting template data.  And, get display logic prior to template.
		Hook::add('viewPreShow',array('View','getViewLogic'));
	}
	
	///searches in the display logic folder and gets all logic for a page, starting with broader and going to narrower scope
	/**For calls with a page/template, it will look in the view/logic/template folder.  For calls without a page/template specified, it will assume controller display logic was desired and look in the view/logic/controller folder*/
	static function getViewLogic(){
		$base = Config::$x['instanceFolder'].'view/logic/controller/';
	...
	*/
	static function getPageLogic($page,$hierarchy=false,$require=true){
		$base = Config::$x['instanceFolder'].'view/logic/template/';
		if(!$hierarchy){
			if($require){
