...
//+	load controllers and page utilities{
		if(!self::$stopControls){
			...
			$pageDisplayUtillityLevel = self::getPageUtilityLevel(Config::$x['instanceFolder'].'view/utilities/section/');
...
//+		include page utilities, if at appropriate level {
				...
				if($pageDisplayUtillityLevel && $level == $pageDisplayUtillityLevel){
					files::incOnce(Config::$x['instanceFolder'].'view/utilities/section/'.implode('/',self::$parsedUrlTokens).'.php');
				}
//+		}
